...
try {
    ...
    $habitat_id = 1; // Default to first habitat
    $conservation_status_id = 1; // Default to first status
    
    // Get habitat ID - map Vietnamese names to database
    $habitat_mapping = [
        'trên cạn' => 'Rừng nhiệt đới',
        'dưới biển' => 'Savanna',
        'trên trời' => 'Rừng taiga',
        'Rừng nhiệt đới' => 'Rừng nhiệt đới',
        'Savanna' => 'Savanna',
        'Rừng taiga' => 'Rừng taiga'
    ];
    
    $habitat_name = $habitat_mapping[$habitat_type] ?? $habitat_type;
    $stmt = $db->prepare("SELECT id FROM habitats WHERE name LIKE ? LIMIT 1");
    $stmt->execute(["%$habitat_name%"]);
    ...
    $result = $stmt->execute([
        $name, $species_id, $description, $habitat_id, 
